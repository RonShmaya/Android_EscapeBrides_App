...
public class Bord {
    ...
    private DataManager dataManager; //TODO: in VER2 remove this class when it's singelton

    public Bord() {

    }
    public Bord(int row, int col, DataManager dataManager , ArrayList<Character> characters) {
        this.rows = row;
        this.cols = col;
    ...
    private void create_board() {
        ...
        for (int row = 0; row < rows; row++) {
            ...
            for (int col = 0; col < rows; col++) {
                places.get(row).add(dataManager.get_position_source(row,col)); //get board
            }
        }
    }
    ...
    public void next_character(Character character) {
        if (character.getDirection() != Moving.Direction.None && character.next_step(rows, cols)) {
            if(!index_taken_by_indexes(character.getLast_row(),character.getLast_col())){
                places.get(character.getLast_row()).get(character.getLast_col()).setImageResource(DataManager.EMPTY_RESOURCE);
            }
            places.get(character.getRow()).get(character.getCol()).setImageResource(character.getCode()); //TODO: add if in coin
        }
    }

    private boolean index_taken_by_indexes(int row , int col) {
    ...
    public void set_charactor_view(Character character, int row, int col, int resId) {
